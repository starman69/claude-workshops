...
3. `workshop/<module>/` — phase folders:
   - `phase-1-install/` — verbatim copy of existing `phase-1-install/NOTES.md`.
   - `phase-2-safety/` — verbatim copies of `NOTES.md`, `before/settings.json`, `after/settings.json`, `after/scripts/block-secrets.sh`. The portable hook path (`$CLAUDE_PROJECT_DIR/.claude/hooks/block-secrets.sh`) needs no changes. If the target has idiosyncratic secrets handling (e.g., intentional fake secrets like Juice Shop), add a callout paragraph in `NOTES.md`.
   - `phase-3-memory/after/CLAUDE.md` — tailored to the target: `<stack>`, `<commands>`, `<key-dirs>`, project-specific gotchas and "don't touch" paths. Include a framing preamble if `<guardrails>` require one.
   - `phase-4-workflows/` — keep `01-find-function.md` and `02-plan-mode.md` (tool-choice drills are target-agnostic). Add 1–2 target-specific exercises if useful.
   - `phase-5-<module>-setup/` — clone, install, run, baseline capture. `before/baseline.md` is a fill-in template; `after/baseline.md` is an example filled baseline; `after/CLAUDE.md.example` is a post-`/init` memory example.
   ...
   - `phase-7-review/NOTES.md` — `/review` (and `/security-review` if applicable), critique, remediation memo, commit.
   - `phase-8-reflection/NOTES.md` — trust-calibration questions appropriate to the target.
...
- Frame every `NOTES.md` as **authorized** training against a project whose maintainers invite vulnerability discovery.
...
- The target project is cloned into a **sibling folder** outside this repo. The module is scaffolding; exercises happen elsewhere. Make sure `NOTES.md` files reflect this (see existing modules for wording).
...
- [ ] `phase-2-safety/after/scripts/block-secrets.sh` is present and executable; hook path in `after/settings.json` is `$CLAUDE_PROJECT_DIR/.claude/hooks/block-secrets.sh`.
- [ ] Both Phase 6 tracks have `before/` (template) and `after/` (filled reference) artifacts.
- [ ] Phase 3 CLAUDE.md includes `<stack>`, `<commands>`, `<key-dirs>`, and (if applicable) the framing preamble from `<guardrails>`.
- [ ] Phase 5 NOTES.md tells the learner to clone **outside this training repo**.
