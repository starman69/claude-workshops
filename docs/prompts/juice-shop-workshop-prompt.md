...
2. `workshop/juice-shop/` — phase folders mirroring the TodoMVC module (order matters — setup precedes safety so the hook and memory land in an existing checkout):
   - `phase-1-install/` — identical to TodoMVC (Claude Code install is module-agnostic).
   - `phase-2-juice-shop-setup/` — clone, `npm install`, run locally on port 3000, confirm the app loads and the built-in challenge tracker (`/#/score-board`) is reachable. Baseline template captures version, Node version, and challenge count. Run `/init` to seed a starter `CLAUDE.md` — the authorized-training preamble gets added in Phase 4.
   - `phase-3-safety/` — same hook and deny-list pattern, but call out that Juice Shop **intentionally** contains dummy secrets, weak JWTs, and sample `.env` files. The `block-secrets.sh` hook still protects the *user's* real credentials; it must not be disabled "because the target is a training app."
   - `phase-4-memory/after/CLAUDE.md` — augments the Phase 2 starter, tailored to Juice Shop: stack (Node/Express/Angular/SQLite+Sequelize), commands (`npm start`, `npm test`, `npm run frisby`), key directories, and an **important preamble stating this is authorized security training against an intentionally vulnerable app**.
   - `phase-5-workflows/` — same tool drills against the cloned Juice Shop source, plus two security-specific exercises: (a) use `Grep` to find all `eval(`, `exec(`, `child_process`, and raw SQL string concatenations; (b) use the `Explore` subagent to map authentication and authorization surfaces.
...
- This is **authorized defensive security training** against a project whose maintainers explicitly invite vulnerability discovery. Keep that framing in every `README.md`.
...
- [ ] Every phase has a `README.md`.
- [ ] `block-secrets.sh` is present under `phase-3-safety/after/scripts/` and executable; hook path in `settings.json` is `$CLAUDE_PROJECT_DIR/.claude/hooks/block-secrets.sh` (portable into the Phase 2 checkout).
