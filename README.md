...
Each module is a self-contained 8-phase workshop (install → safety → memory → workflows → target setup → demo → review → reflection). Phase 6 splits into two tracks so facilitators can match the learner's goal.
...
4. Work through each phase's `NOTES.md` in order. For phases with `before/after`: copy `before/` into your working area, do the exercise, then `diff` against `after/`.
...
workshop/
  ...
  <module>/
    ...
    phase-N-<name>/     NOTES.md + before/after snapshots
...
Read [`CLAUDE.md`](CLAUDE.md) first — it's the style guide and the guardrail list (`NOTES.md` ≤60 lines, no working files in the repo root, `before/after` file naming conventions, etc.). `CLAUDE.md` targets Claude Code but applies to human contributors too.
